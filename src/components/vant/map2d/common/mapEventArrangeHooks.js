...
export function mapEventArrangeHooks() {
  ...
  const fitLayerPosition = bboxArray => {
    if (!bboxArray) {
      AppInstance.$vantNotify({ message: "图层没有边界，请手动设置中心点和缩放层级", type: "danger" });
      return;
    }
  ...
  const set2DMapCenter = lnglatString => {
    ...
    if (lnglatArr.length < 2) {
      AppInstance.$vantNotify({ message: "请输入正确的查询坐标！", type: "danger" });
      return;
    }
  ...
  const focus2DMapLayer = layerName => {
    ...
    if (!foucsLayer) {
      AppInstance.$vantNotify({ message: "图层获取失败，请检查图层名称是否正确", type: "danger" });
      return;
    }
    ...
    AppInstance.$vantNotify({ message: "获取不到图层的边界，请手动设置中心点和缩放层级", type: "danger" });
  };
  // 聚焦要素
  ...
  const add2DMapLayer = (layerType, serviceType, layerName, layerUrl) => {
    try {
      ...
      AppInstance.$vantNotify({ message: `${err}配置错误，请联系管理员处理`, type: "danger" });
    }
  };
  // 通过图层名称删除一个图层
