<template>
  <div class="dynamic-form-box">
    <van-form ref="vantDynamicFormRef">
      <template v-if="formTableData.tableList.length">
        <!-- 表单项 -->
        <div
          class="form-item"
          v-for="(tableItem, index) in formTableData.tableList"
          :key="index"
        >
          ...
          <div class="operate-box" v-if="showOperateButton(index)">
            <template v-for="buttonItem in buttonData">
              <van-button
                ...
                @click="
                  handleOnOperate(tableItem, index, buttonItem.operateType)
                "
              >
                {{ buttonItem.text }}
              </van-button>
...
const vantDynamicFormRef = ref(null);
defineExpose({
  vantDynamicFormRef,
  formTableData,
