...
let defaultAttrs = computed(() => {
  return Object.assign({}, defaultHighchartAttribute, barLineAttribute);
});
...
async function initChart(resResultItem) {
  ...
  await setBarLineData(option, resResultItem);
  renderOptions.value = option;
  renderRef.value.initRenderChart();
}
async function setBarLineData(option, resResultItem) {
  let resultItem = [];
  if (resResultItem) {
  ...
  optionItem.forEach((item, index) => {
    ...
    let seriesObject = {
      name: item.catalog,
      type: item.type ? item.type : "column",
      color: barLineColor,
      data: []
    };
