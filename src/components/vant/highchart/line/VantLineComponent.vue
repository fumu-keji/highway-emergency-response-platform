...
let defaultAttrs = computed(() => {
  return Object.assign({}, defaultHighchartAttribute, lineAttribute);
});
...
async function initChart(resResultItem) {
  ...
  let option = {
    chart: { type: Attr.smoothLine ? "spline" : "line" },
  ...
  await setLineData(option, resResultItem);
  renderOptions.value = option;
  renderRef.value.initRenderChart();
}
async function setLineData(option, resResultItem) {
  let resultItem = [];
  if (resResultItem) {
